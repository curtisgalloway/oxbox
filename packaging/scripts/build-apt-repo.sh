...
# Assemble the signed apt repository that apt.yml publishes to GitHub Pages
...
# Release/InRelease, and the public key as oxbox.asc — into the output dir.
...
COMPONENT=main
ARCHES="amd64 arm64"
BASE_URL="https://curtisgalloway.github.io/oxbox/apt"
...
# page cannot drift from the key apt.yml actually imported.
...
# replayed index can be served. 30 days; apt.yml rebuilds weekly on a
...
apt-ftparchive \
  -o "APT::FTPArchive::Release::Origin=oxbox" \
  -o "APT::FTPArchive::Release::Label=oxbox" \
...
gpg --batch --armor --export "$KEY" > oxbox.asc

cat > index.html <<EOF
<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"><title>oxbox apt repository</title></head>
<body><h1>oxbox apt repository</h1>
<p>Signed apt repository for <a href="https://github.com/curtisgalloway/oxbox">oxbox</a>
(amd64/arm64, Debian 12+ and Raspberry Pi OS). To use it:</p>
<pre>
sudo install -d /etc/apt/keyrings
sudo curl -fsSL -o /etc/apt/keyrings/oxbox.asc $BASE_URL/oxbox.asc
sudo tee /etc/apt/sources.list.d/oxbox.sources &gt;/dev/null &lt;&lt;'SRC'
Types: deb
URIs: $BASE_URL
Suites: $SUITE
